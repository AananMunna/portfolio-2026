      ...
      <div className="w-full h-full bg-[#111] rounded-2xl p-5 flex flex-col gap-4 text-white relative overflow-hidden">
        {isReact && (
          <div className="absolute inset-0 pointer-events-none z-50 p-2 opacity-20">
            <div className="border border-dashed border-accent rounded-xl w-full h-full relative">
              <span className="absolute -top-3 left-2 bg-[#111] text-accent text-[10px] font-mono px-1">&lt;ProfileCard /&gt;</span>
            </div>
          </div>
        )}
        <div className="flex items-center gap-3 relative z-10">
          <div className="relative">
            <img src="https://picsum.photos/seed/aanan/150/150" alt="Avatar" className="rounded-full border-2 border-white/10 object-cover" style={{ width: 52, height: 52 }} />
            {isReact && <div className="absolute -inset-1 border border-dashed border-accent rounded-full opacity-25 pointer-events-none" />}
          </div>
          <div>
            <h3 className="font-display text-base font-bold flex items-center gap-1.5">
              AANAN {dbSynced && <CheckCircle2 className="w-3.5 h-3.5 text-accent" />}
            </h3>
            <p className="text-muted text-xs font-mono">Frontend Specialist | MERN</p>
          </div>
        </div>
        <p className="text-gray-300 text-xs leading-relaxed relative z-10">Crafting pixel-perfect UIs, powered by robust backends.</p>
        <div className="flex gap-4 text-xs font-mono text-muted relative z-10">
          <div><span className="text-white font-bold">{isFollowing ? "1,025" : "1,024"}</span> Followers</div>
        ...
        <div className="flex gap-2 mt-auto relative z-10">
          ...
          {isReact ? (
            ...
            >
              <Heart className={`w-3.5 h-3.5 ${likes > 0 ? "fill-red-500 text-red-500" : ""}`} />
              <div className="absolute -inset-1 border border-dashed border-accent rounded-lg opacity-20 pointer-events-none" />
            </motion.button>
          ) : (
            <button
      ...
      {steps.map((step, index) => {
        ...
        return (
          <motion.button
            ...
            className={`text-left p-4 rounded-2xl border transition-colors duration-200 relative overflow-hidden w-full
              ${isActive
          ...
          >
            ...
            <div className="flex items-start gap-3 relative z-10">
              <div className={`p-2.5 rounded-xl ${isActive ? step.bgColor : "bg-white/5"}`}>
                <Icon className={`w-5 h-5 ${isActive ? step.color : "text-muted"}`} />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-0.5">
                  <h3 className={`font-bold text-base ${isActive ? "text-white" : "text-gray-400"}`}>{step.name}</h3>
                  <span className={`text-xs font-mono px-2 py-0.5 rounded-full ${isActive ? `${step.bgColor} ${step.color}` : "bg-white/5 text-muted"}`}>
                    {step.tech}
                  </span>
                </div>
            ...
            {isActive && <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-accent rounded-l-2xl" />}
          </motion.button>
        );
      })}

      <div className="flex justify-between items-center mt-2 px-1">
        <button onClick={handleReset} className="text-xs font-mono text-muted hover:text-white transition-colors flex items-center gap-1.5">
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-5 lg:hidden">
          ...
          {/* Preview card — full width, no scale */}
          <div className="w-full rounded-2xl border border-white/10 bg-[#050505] overflow-hidden shadow-xl">
            {/* hint bar */}
            <div className="px-4 py-2.5 border-b border-white/10 flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-accent flex-shrink-0" />
              <p className="text-xs font-mono text-muted truncate">{hintText[activeStep]}</p>
            </div>

            {/* The actual preview — full size, no transform */}
            <div className="w-full h-[340px] min-[400px]:h-[380px]">
              ...
              <div className="flex gap-1.5">
                {steps.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${i === activeStep ? "w-6 bg-accent" : i < activeStep ? "w-1.5 bg-accent/40" : "w-1.5 bg-white/10"}`}
                  />
                ))}
              </div>
        ...
        <div className="hidden lg:flex gap-12 items-start relative">
          {/* Left: Steps */}
          ...
          {/* Right: Visualization */}
          <div className="w-7/12 h-[650px] sticky top-24 z-40 rounded-3xl border border-white/10 bg-[#050505] overflow-hidden shadow-2xl flex items-center justify-center">
            <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px]" />

            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] flex items-center justify-center gap-8 scale-100">
              <div className={`relative z-20 flex-shrink-0 transition-all duration-500 ease-in-out ${activeStep >= 4 ? "w-[280px] h-[380px]" : "w-[340px] h-[460px]"}`}>
                <div className="absolute -top-7 left-0 text-xs font-mono text-muted flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-accent" /> Browser (Frontend)
                </div>
                ...
                {activeStep >= 4 && (
                  ...
                  >
                    <div className="absolute -top-7 left-1/2 -translate-x-1/2 text-xs font-mono text-muted flex items-center gap-2 whitespace-nowrap">
                      <div className="w-2 h-2 rounded-full bg-accent" /> Server (Backend)
                    </div>
                    <div className={`w-full h-full bg-[#111] rounded-2xl border-2 flex flex-col items-center justify-center gap-4 transition-colors duration-300 ${isPinging ? "border-accent" : "border-white/10"}`}>
                      <Server className={`w-12 h-12 transition-colors duration-300 ${isPinging ? "text-accent" : "text-muted"}`} />
                      <div className="text-xs font-mono text-center text-muted">Express.js<br />API Gateway</div>
                      <AnimatePresence>
                        {isPinging && (
                    ...
                    <div className="absolute top-1/2 -left-8 w-8 h-px bg-white/10 -translate-y-1/2 overflow-visible">
                      <AnimatePresence>
                        {isPinging && (
                          <motion.div initial={{ left: 0, opacity: 1 }} animate={{ left: "100%", opacity: 0 }} transition={{ duration: 0.4 }}
              ...
              <AnimatePresence>
                {activeStep >= 5 && (
                  ...
                  >
                    <div className="absolute -top-7 left-1/2 -translate-x-1/2 text-xs font-mono text-muted flex items-center gap-2 whitespace-nowrap">
                      <div className="w-2 h-2 rounded-full bg-accent" /> Database
                    </div>
                    <div className={`w-full h-full bg-[#111] rounded-2xl border-2 flex flex-col items-center justify-center gap-4 transition-colors duration-300 ${dbSynced ? "border-accent" : "border-white/10"}`}>
                      <Database className={`w-10 h-10 transition-colors duration-300 ${dbSynced ? "text-accent" : "text-muted"}`} />
                      <div className="text-xs font-mono text-center text-muted">MongoDB<br />Storage</div>
                    </div>
                    <div className="absolute top-1/2 -left-8 w-8 h-px bg-white/10 -translate-y-1/2 overflow-visible">
                      <AnimatePresence>
                        {isPinging && (
                          <motion.div initial={{ left: 0, opacity: 1 }} animate={{ left: "100%", opacity: 0 }} transition={{ duration: 0.4, delay: 0.12 }}
                            className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-accent rounded-full" />
                        )}
                      </AnimatePresence>
                    </div>
            ...
            {/* Bottom hint */}
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full text-xs font-mono text-white text-center w-[90%] max-w-sm z-30">
              {hintText[activeStep]}
            </div>
          </div>
        </div>
        ...
        {toastVisible && (
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-accent/10 flex items-center justify-center flex-shrink-0">
              {toastStep === 0 ? <FileCode2 className="w-5 h-5 text-accent" /> : <Paintbrush className="w-5 h-5 text-accent" />}
            </div>
            <div>
              <p className="text-white text-sm font-bold font-mono">No JavaScript yet!</p>
